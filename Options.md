...
| **crawl_interval** | *(varies)* | `125` | Crawl movement interval in milliseconds |
...
| **last_item_show_duration** | *(varies)* | `3` | Duration for showing last item in context menu |
...
| **move_interval** | *(varies)* | `50` | Normal movement interval in milliseconds |
| **runmode** | `teleport` / `run` / `walk` / `crawl` | `run` | Display frequency when `running' or `travelling' |
| **shield_effect_length** | *(varies)* | `10` | Shield effect length |
...
| **sortloot** | `none` / `loot` / `full` | `loot` | Sort object selection lists by description |
...
| **talk_effect_length** | *(varies)* | `6` | Talk effect length |
...
| **disclose** | `iavgco` options | `ni na nv ng nc no` | The kinds of information to disclose at end of game |
...
| **max_hint_difficulty** | *(varies)* | `veteran (-1)` | Maximum difficulty level for showing hints |
...
| **packorder** | *(varies)* | ``$")[8=%?+!(9*7`0_`` | The inventory order of the items in your pack |
| **paranoid_confirmation** | *(varies)* | `pray Autoall multishot` | Extra prompting in certain situations |
| **pickup_burden** | `unencumbered` / `burdened` / `stressed` / `strained` / `overtaxed` / `overloaded` | `stressed` | Maximum burden picked up before prompt |
| **pickup_thrown** | `true` / `false` | `true` | Autopickup thrown items |
| **pickup_types** | *(varies)* | `all` | Types of objects to pick up automatically |
| **pile_limit** | *(varies)* | `5` | Threshold for 'there are many objects here' |
| **player_selection** | `dialog` / `prompts` | `dialog` | Choose character via dialog or prompts |
| **playmode** | `normal` / `explore` / `debug` / `casual` / `modern` / `reloadable` | `normal` | Classic or modern play, non-scoring explore or casual mode, or debug mode |
...
| **scores** | *(varies)* | `3 top/2 around` | The parts of the score list you wish to see |
...
| **spellorder** | `0` to `7` | `0` | Default spell sorting |
...
| **middle_click_command** | `default` / `by role` / `?` / `off` / `look` / `move` / `cast` / `fire` / `zap` | `by role` | Command upon clicking middle mouse button |
| **mouse_support** | `0` (off) / `1` (on, OS adjusted) / `2` (on, OS unchanged) | `0` | Game receives click info from mouse |
| **number_pad** | `-2` (off, yuhj) / `-1` (off, y/z swapped) / `0` (off) / `1` (on) / `2` (on, MSDOS) / `3` (on, phone-style) / `4` (on, phone MSDOS) | `0` | Use the number pad for movement |
...
| **right_click_command** | `default` / `by role` / `?` / `off` / `look` / `move` / `cast` / `fire` / `zap` | `by role` | Command upon clicking right mouse button |
| **run_spot_distance** | *(varies)* | `10` | Longest distance for spotting a monster to stop running or travelling |
...
| **whatis_filter** | `none` / `view` / `area` | `none` | Filter coordinate locations when targeting next or previous |
...
| **catgender** | `male` / `female` / `random` | `random` | The gender of your (first) cat (e.g., catgender:female) |
...
| **doggender** | `male` / `female` / `random` | `random` | The gender of your (first) dog (e.g., doggender:male) |
| **dogname** | *(varies)* | *(none)* | The name of your (first) dog (e.g., dogname:Fang) |
| **horsegender** | `male` / `female` / `random` | `random` | The gender of your (first) horse (e.g., horsegender:female) |
...
| **pettype** | `cat` / `dog` / `horse` / `none` / `random` | `random` | Your preferred initial pet type |
| **ramgender** | `male` / `female` / `random` | `random` | The gender of your (first) ram (e.g., ramgender:male) |
| **ramname** | *(varies)* | *(none)* | The name of your (first) ram (e.g., ramname:Silver) |
| **safe_pet** | `true` / `false` | `true` | Prevent you from hitting pets |
| **wolfgender** | `male` / `female` / `random` | `random` | The gender of your (first) direwolf (e.g., wolfgender:female) |
...
| Option Name | Values | Default | Description |
| :---------- | :----: | :-----: | :---------- |
| **align** | `lawful` / `neutral` / `chaotic` / `random` | `random` | Your starting alignment (lawful, neutral, or chaotic) |
| **blind** | `true` / `false` | `false` | Your character is permanently blind |
| **female** | `true` / `false` | `false` | Is your character female |
| **fruit** | *(varies)* | `slime mold` | The name of a fruit you enjoy eating |
| **gender** | `male` / `female` / `random` | `random` | Your starting gender (male or female) |
| **luggagename** | *(varies)* | *(none)* | The name of your (first) luggage (e.g., luggagename:Albert) |
| **name** | *(varies)* | *(none)* | Your character's name (e.g., name:Merlin-W) |
| **nudist** | `true` / `false` | `false` | Start your character without armor |
| **race** | *(varies)* | `random` | Your starting race (e.g., Human, Elf) |
| **role** | *(varies)* | `random` | Your starting role (e.g., Barbarian, Valkyrie) |
...
| Option Name | Values | Default | Description |
| :---------- | :----: | :-----: | :---------- |
| **animation_interval** | *(varies)* | `25` | Animation frame interval in milliseconds |
| **extmenu** | `true` / `false` | `false` | Use menu for getting extended commands (supported by TTY/ASCII, Curses, and Legacy Windows GUI) |
...
| **menustyle** | `traditional` / `combination` / `full` / `partial` | `full` | User interface for object selection (supported by TTY/ASCII, Curses, and Legacy Windows GUI) |
| **msg_window** | `single` / `combination` / `full` / `reversed` | `full` (TTY) / `reversed` (Curses) | The type of message window required LIFO/FIFO (supported by TTY/ASCII and Curses) |
...
| **altkeyhandler** | *(varies)* | `default` | Alternate key handler |
| **altmeta** | `true` / `false` | `false` | Treat 'ESC c' as M-c |
...
| **petattr** | *(varies)* | *(none)* | Attributes for highlighting pets |
...
| **whatis_coord** | `none` / `map` / `compass` / `full compass` / `screen` | `none` | Show coordinates when auto-describing cursor position |
...
| **align_status** | *(varies)* | `bottom` | Status window alignment |
...
| **here_window_size** | *(varies)* | `8` | Number of rows in the here window |
...
| **statuslines** | *(varies)* | `2` | 2 to 8 lines for status display |
...
| **windowborders** | `0` (off) / `1` (on) / `2` (auto) | `2` (auto) | 0 (off), 1 (on), 2 (auto) |
...
| **sound_volume_ambient** | *(varies)* | `50` | Ambient volume |
| **sound_volume_dialogue** | *(varies)* | `50` | Dialogue volume |
| **sound_volume_effects** | *(varies)* | `50` | Sound effect volume |
| **sound_volume_general** | *(varies)* | `100` | General game volume |
| **sound_volume_music** | *(varies)* | `50` | Music volume |
| **sound_volume_ui** | *(varies)* | `50` | User interface sound volume |
