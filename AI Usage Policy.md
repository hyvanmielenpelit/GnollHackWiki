![AI Usage Policy](/uploads/AI%20Usage%20Policy/ai-usage-policy-q85.webp)

> 👉 **This policy outlines the rules and guidelines for using generative artificial intelligence (AI) in the development of GnollHack, covering game assets, programming code, and documentation.**

## 🎨 Generative AI Assets

> ℹ️ This section explains the details of how we use assets created with generative AI in GnollHack-related contexts.

| AI Asset Type ↓ / Use Allowed → | GnollHack | Stores<sup>1</sup> | Gnoll Overseer | Other Contexts<sup>2</sup> |
| :------------------------------ | :-------: | :---------------------: | :------------: | :------------------------: |
| Image | ❌ | ❌<sup>3</sup> / ✅<sup>4</sup> | ✅ | ✅ |
| Text<sup>5, 6</sup> | ❌ | ✅ | ✅ | ✅ |
| Voice | ❌ | ❌ | ✅ | ✅ |
| Music | ❌ | ❌ | ❌ | ✅ |
| Video | ❌ | ❌ | ✅ | ✅ |
| 3D Model | ❌ | ❌ | ✅ | ✅ |

- <sup>1</sup> This includes store page assets and other assets in stores, such as the Google Play Store.
- <sup>2</sup> This includes, but is not limited to, advertising and the wiki.
- <sup>3</sup> Store page assets.
- <sup>4</sup> Other assets, such as event graphics.
- <sup>5</sup> AI may be used to check the grammar of English text, for example.
- <sup>6</sup> Text in stores is usually a mix of AI-generated and human-generated text.

## 💻 AI Programming

> ℹ️ We use generative AI to *assist* our development, especially to help us complete complex and/or tedious programming tasks.

Programmers may use generative AI, including AI agents, to:

1. Generate code
2. Complete programming workflows and tasks more efficiently
3. Debug the game
4. Generate and review implementation plans
5. Review the code

## 📚 AI Documentation

> ℹ️ We use generative AI to create accurate documentation quickly.

Generative AI, including AI agents, may be used to:

1. Generate documentation (e.g., document the game code and create pages in the GnollHack Wiki)
2. Verify that documentation is correct (i.e., compare documentation with the game code)
3. Check and correct the grammar of documentation
4. Apply styling to documentation
5. Organize documentation
